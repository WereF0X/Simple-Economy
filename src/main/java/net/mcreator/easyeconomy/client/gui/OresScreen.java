...
import net.mcreator.easyeconomy.network.OresButtonMessage;
...
public class OresScreen extends AbstractContainerScreen<OresMenu> {
	...
	private static final ResourceLocation texture = new ResourceLocation("easy_economy:textures/screens/ores.png");
	...
	public void init() {
		super.init();
		button_iron = Button.builder(Component.translatable("gui.easy_economy.ores.button_iron"), e -> {
			if (true) {
				EasyEconomyMod.PACKET_HANDLER.sendToServer(new OresButtonMessage(0, x, y, z));
		...
		button_coal = Button.builder(Component.translatable("gui.easy_economy.ores.button_coal"), e -> {
			if (true) {
				EasyEconomyMod.PACKET_HANDLER.sendToServer(new OresButtonMessage(1, x, y, z));
		...
		button_gold = Button.builder(Component.translatable("gui.easy_economy.ores.button_gold"), e -> {
			if (true) {
				EasyEconomyMod.PACKET_HANDLER.sendToServer(new OresButtonMessage(2, x, y, z));
		...
		button_emerald = Button.builder(Component.translatable("gui.easy_economy.ores.button_emerald"), e -> {
			if (true) {
				EasyEconomyMod.PACKET_HANDLER.sendToServer(new OresButtonMessage(3, x, y, z));
		...
		button_diamond = Button.builder(Component.translatable("gui.easy_economy.ores.button_diamond"), e -> {
			if (true) {
				EasyEconomyMod.PACKET_HANDLER.sendToServer(new OresButtonMessage(4, x, y, z));
		...
		button_back = Button.builder(Component.translatable("gui.easy_economy.ores.button_back"), e -> {
			if (true) {
				EasyEconomyMod.PACKET_HANDLER.sendToServer(new OresButtonMessage(5, x, y, z));
				OresButtonMessage.handleButtonAction(entity, 5, x, y, z);
			}
		}).bounds(this.leftPos + 1, this.topPos + 2, 46, 20).build();
		guistate.put("button:button_back", button_back);
		this.addRenderableWidget(button_back);
		button_netherite = Button.builder(Component.translatable("gui.easy_economy.ores.button_netherite"), e -> {
			if (true) {
				EasyEconomyMod.PACKET_HANDLER.sendToServer(new OresButtonMessage(6, x, y, z));
